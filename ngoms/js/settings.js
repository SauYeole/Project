...
$(document).ready(function () {
  ...
  $(".theme-settings input").on("ifClicked", function () {
    ...
    if (input.attr("name") === "st_sb_scroll") {
      if (
        theme_settings.st_sb_scroll == 1 &&
        theme_settings.st_layout_boxed == 0
      ) {
        theme_settings.st_sb_fixed = 1;
      } else if (
        theme_settings.st_sb_scroll == 1 &&
        theme_settings.st_layout_boxed == 1
      ) {
        theme_settings.st_sb_fixed = -1;
      } else if (
        theme_settings.st_sb_scroll == 0 &&
...
function set_settings(theme_settings, option) {
  ...
  else $(".page-container").removeClass("page-navigation-top-fixed");
  /* END Header Fixed */

  /* Start Sidebar Fixed */
  if (theme_settings.st_sb_fixed == 1) {
    $(".page-sidebar").addClass("page-sidebar-fixed");
  } else $(".page-sidebar").removeClass("page-sidebar-fixed");
  /* END Sidebar Fixed */
  ...
  else $(".page-container").removeClass("page-mode-rtl");
  /* END Right Sidebar */

  /* Start Custom Sidebar */
  if (theme_settings.st_sb_custom == 1)
    $(".page-sidebar .x-navigation").addClass("x-navigation-custom");
  else $(".page-sidebar .x-navigation").removeClass("x-navigation-custom");
  /* END Custom Sidebar */
  ...
  /* Start Layout Boxed */
  if (theme_settings.st_layout_boxed == 1) $("body").addClass("page-container");
  else $("body").removeClass("page-container");
  /* END Layout Boxed */

  /* Set states for options */
  if (
    option === false ||
    option === "st_layout_boxed" ||
    option === "st_sb_fixed" ||
    option === "st_sb_scroll"
  ) {
